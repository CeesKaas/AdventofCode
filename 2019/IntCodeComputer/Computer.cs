...
namespace IntCodeComputer
{
    public class Computer
    {
        ...
        {
            _memoryState = program.Select((value, index) => (index, value)).ToDictionary(_ => (long)_.index, _ => _.value);
            _input = input;
            _output = output;
        }
        ...
        public void Start()
        {
            _thread = Task.Factory.StartNew(() =>
            {
                ...
                while (true)
                {
                    ...
                    var op = GetParameterValue(instructionPointer, ParameterMode.Immediate);
                    long opcode = op % 100;
                    ParameterMode parameterMode1 = (ParameterMode)((op / 100) % 10);
                    ParameterMode parameterMode2 = (ParameterMode)((op / 1000) % 10);
                    ParameterMode parameterMode3 = (ParameterMode)((op / 10000) % 10);
                    long step;
                    switch (opcode)
                    {
                        case 1:
